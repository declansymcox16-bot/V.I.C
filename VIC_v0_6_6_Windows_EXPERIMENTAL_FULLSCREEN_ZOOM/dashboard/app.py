...
app = Flask(__name__)
app.secret_key = "vic-v0.6.6 EXPERIMENTAL FULLSCREEN + ZOOM-local-dashboard"

DASHBOARD_VERSION = "0.6.6"
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.6.6 EXPERIMENTAL FULLSCREEN + ZOOM folder."
        )
    return True, ""
...
(function(){
  ...
  function applyZoom(value,shouldSave){
    current=clamp(value);
    if(root){
      root.style.zoom=String(current/100);
...
if __name__ == "__main__":
    ...
    print("VIC dashboard v0.6.6 EXPERIMENTAL FULLSCREEN + ZOOM")

...
app = Flask(__name__)
app.secret_key = "vic-v0.6.9 EXPERIMENTAL BULK SOURCE CONTROL-local-dashboard"

DASHBOARD_VERSION = "0.6.9"
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.6.9 EXPERIMENTAL BULK SOURCE CONTROL folder."
        )
    return True, ""
...
def manage_sources_page():
    ...
    body = """<div class='card'><div class='live-status-row'><div><h2 style='margin:0'>Source Library</h2><p class='muted'>Disable sources temporarily, archive them without deleting settings, duplicate them, or export/import portable source files.</p><p class='small'><strong>Enable All:</strong> enables every non-archived source. <strong>Disable All:</strong> disables every inactive source; active previews or recordings are safely skipped. <strong>Export one:</strong> press <em>Export Source</em> in that source's Actions column.</p></div><div class='inline'><a class='btn' href='/add'>+ Add source</a><form method='post' action='/sources/enable-all' onsubmit='return confirm("Enable every non-archived source? Archived sources will remain archived.")'><button class='control-start'>Enable All</button></form><form method='post' action='/sources/disable-all' onsubmit='return confirm("Disable all sources? Any source currently previewing, testing or recording will be left enabled until it is stopped.")'><button class='control-stop'>Disable All</button></form><a class='btn' href='/sources/import'>Import Source / ZIP</a><a class='btn' href='/sources/export-all'>Export All Sources ZIP</a></div></div></div>
...
if __name__ == "__main__":
    ...
    print("VIC dashboard v0.6.9 EXPERIMENTAL BULK SOURCE CONTROL")

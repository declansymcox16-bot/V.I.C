...
app = Flask(__name__)
app.secret_key = "vic-v0.8.1 EXPERIMENTAL WORKER SETUP REPAIR-local-dashboard"

DASHBOARD_VERSION = "0.8.1"
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.8.1 EXPERIMENTAL WORKER SETUP REPAIR folder."
        )
    return True, ""
...
def build_worker_copy_zip() -> Path:
    stamp = time.strftime("%Y-%m-%d_%H-%M-%S")
    target = EXPORTS_DIR / f"VIC_Worker_Copy_v0.8.1_{stamp}.zip"
...
if __name__ == "__main__":
    ...
    print("VIC dashboard v0.8.1 EXPERIMENTAL WORKER SETUP REPAIR")

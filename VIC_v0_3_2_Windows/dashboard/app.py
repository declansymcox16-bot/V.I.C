...
app = Flask(__name__)
app.secret_key = "vic-v0.3.2-local-dashboard"
...
<header><h1>VIC — Video Ingest Cluster v0.3.2</h1><nav><a href='/'>Dashboard</a><a href='/add'>+ Add source</a><a href='/mass-capture'>Capture Everything</a><a href='/workers'>Workers</a><a href='/live'>Live</a><a href='/recordings'>Recordings</a><a href='/jobs'>Jobs</a><a href='/help'>Help</a></nav></header><main>
...
async function loadDevices(){let id=document.getElementById('workerSelect').value;if(id==='auto'){const r=await fetch('/api/automatic-worker');const d=await r.json();if(!d.id){alert('No online worker. Start VIC and wait for Local PC to appear.');return;}id=d.id;}const r=await fetch('/api/workers/'+id+'/inventory');const d=await r.json();if(d.error){alert(d.error);return;}const v=document.getElementById('videoDevices'),a=document.getElementById('audioDevices'),s=document.getElementById('screenSelect'),sp=document.getElementById('speakerSelect');if(v){v.innerHTML='';(d.video||[]).forEach(x=>v.appendChild(new Option(x,x)));}if(a){a.innerHTML='';(d.audio_inputs||d.audio||[]).forEach(x=>a.appendChild(new Option(x,x)));}if(s){s.innerHTML='';(d.screens||[]).forEach(x=>s.appendChild(new Option(x.label||x.name,x.id)));if(!(d.screens||[]).length)s.appendChild(new Option('No monitors reported',''));}if(sp){sp.innerHTML='';(d.speakers||[]).forEach(x=>sp.appendChild(new Option(x.label||x.name,x.id)));if(!(d.speakers||[]).length)sp.appendChild(new Option('No speaker outputs reported',''));sp.onchange=()=>{const chosen=(d.speakers||[]).find(x=>x.id===sp.value);const hidden=document.getElementById('speakerName');if(hidden)hidden.value=chosen?chosen.name:'';};sp.dispatchEvent(new Event('change'));}const summary=document.getElementById('deviceSummary');if(summary)summary.textContent='Loaded '+(d.screens||[]).length+' monitor(s), '+(d.video||[]).length+' video device(s), '+(d.audio_inputs||d.audio||[]).length+' microphone/input device(s), and '+(d.speakers||[]).length+' speaker/output device(s).';}
</script>"""
    return render_template_string(

...
app = Flask(__name__)
app.secret_key = "vic-v0.5.0-local-dashboard"
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.5.0 folder."
        )
    return True, ""
...
{% if transfers %}<div class='card'><div class='live-status-row'><div><h3 style='margin:0'>File-transfer queue</h3><p class='muted'>Each worker can handle several transfers at the same time. Files in a mass operation remain individually verified, so one failed file does not remove any unverified original.</p></div><form method='post' action='/transfers/clear-finished'><button>Clear finished transfers</button></form></div>

...
# Worker Setup can choose 1-6 simultaneous transfers per worker.
...
def load_config() -> dict[str, Any]:
    ...
    if "transfer_parallel_limit" not in cfg:
        cfg["transfer_parallel_limit"] = 3
        changed = True
    try:
        cfg["transfer_parallel_limit"] = max(
            1,
            min(6, int(cfg.get("transfer_parallel_limit", 3))),
        )

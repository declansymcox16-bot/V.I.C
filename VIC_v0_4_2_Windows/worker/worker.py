...
def start_job(cfg: dict[str, Any], ffmpeg: str | None, job: dict[str, Any]) -> None:
    ...
    with ACTIVE_LOCK:
        if job["id"] in ACTIVE:
            return
        if (
            job.get("mode") in transfer_modes
            and any(
                bool(local.get("transfer_job"))
                for local in ACTIVE.values()
            )
        ):
            # Keep this job pending. The next heartbeat will retry it after
            # the earlier transfer thread has finished.
            return

...
def start_screen_speaker_job(cfg: dict[str, Any], ffmpeg: str | None, job: dict[str, Any]) -> None:
    """Record screen video and selected Windows speaker loopback as separate synchronized files."""
    if not ffmpeg:
    ...
    if test_mode:
        ...
        output_text = "3-second screen + speaker loopback test"
    else:
        command += [
            "-map", "0:v:0",
            "-c:v", "libx264",
    ...
    def audio_loop() -> None:
        ...
        try:
            ...
            update_job(
                ...
                state="running",
                message=(
                    f"Testing screen and speaker loopback: {speaker_name}"
                    if test_mode
                    else f"Recording screen plus speaker loopback: {speaker_name}. Audio is saved as a companion WAV file."
                ),
                output=output_text,
                audio_level_db=-70.0,
            ...
            if test_mode:
                ...
                update_job(
                    ...
                    job["id"],
                    state="finished",
                    message=f"Screen and speaker loopback test passed. Latest level {local['audio_level_db']} dB.",
                    output=output_text,
            ...
            update_job(
                ...
                job["id"],
                state="failed",
                message=f"Screen speaker-loopback capture failed: {exc}",
                output=output_text,
    ...
    thread = threading.Thread(
        target=audio_loop,
        daemon=True,
        name=f"VIC-ScreenSpeaker-{job['id'][:8]}",
    )
    local["thread"] = thread
    with ACTIVE_LOCK:
        ACTIVE[job["id"]] = local

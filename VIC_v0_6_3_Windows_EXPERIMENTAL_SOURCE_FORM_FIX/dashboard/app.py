...
app = Flask(__name__)
app.secret_key = "vic-v0.6.3 EXPERIMENTAL SOURCE FORM FIX-local-dashboard"

DASHBOARD_VERSION = "0.6.3"
...
PREVIEW_QUALITY_PRESETS = {
    ...
    "high": {"width": 960, "jpeg_quality": 4, "label": "High — 960 px"},
    "very_high": {"width": 1280, "jpeg_quality": 2, "label": "Very high — 1280 px"},
}
...
def settings() -> dict[str, Any]:
    ...
    try:
        result["preview_width"] = max(
            160,
            min(1920, int(result.get("preview_width", 640))),
        )
    except (TypeError, ValueError):
    ...
    result["audio_meter_smoothing"] = bool(
        result.get("audio_meter_smoothing", True)
    )
    return result
...
def runtime_live_settings() -> dict[str, Any]:
    ...
    return {
        ...
        "audio_meter_smoothing": bool(
            cfg.get("audio_meter_smoothing", True)
        ),
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.6.3 EXPERIMENTAL SOURCE FORM FIX folder."
        )
    return True, ""
...
const dashboardSmooth={{audio_smoothing|tojson}};
function dashboardSetMeter(meter,percent){if(!meter)return;meter.style.transitionDuration=dashboardSmooth?Math.min(1000,Math.max(40,dashboardAudioMs*0.9))+'ms':'0ms';meter.style.width=Math.max(0,Math.min(100,Number(percent)||0))+'%';}
...
def source_form_script(
    ...
    setup_audio_smoothing_json = json.dumps(
        bool(live_cfg["audio_meter_smoothing"])
    )
    return f"""<script>
...
async function refreshAudioMeters(){{
  ...
  try{{
    ...
    audioMeterJobs.forEach((jobId,rowId)=>{{
      ...
      const text=document.getElementById(rowId+'-text');
      const status=document.getElementById(rowId+'-status');
      if(meter){{meter.style.transitionDuration=(setupAudioSmoothing?Math.min(1000,Math.max(40,setupAudioRefreshMs*0.9)):0)+'ms';meter.style.width=(item.audio_percent||0)+'%';}}
...
const setupAudioSmoothing={setup_audio_smoothing_json};
    ...
    with zipfile.ZipFile(
        ...
        archive.writestr(
            ...
            (
                ...
                "=========================\n\n"
                "Import one source by opening Source Library, pressing "
                "Import Source and choosing its .vicsource.json file.\n"
                "Imported sources start disabled so their worker and device "
                "settings can be checked safely.\n"
            ),
        )
...
const audioSmooth={{audio_smoothing|tojson}};
let lastAudioAt=0;
function setLiveListMeter(meter,percent){if(!meter)return;meter.style.transitionDuration=audioSmooth?Math.min(1000,Math.max(40,audioMs*0.9))+'ms':'0ms';meter.style.width=(Number(percent)||0)+'%';}
...
<div class='card'><h3>Audio visual refresh</h3><label>Preset</label><select id='audioRefreshPreset' name='audio_refresh_preset' onchange='refreshPresetChanged("audio")'><option value='instant' {{"selected" if cfg.audio_meter_refresh_preset=="instant" else ""}}>Instant — 0.00 seconds</option><option value='fast' {{"selected" if cfg.audio_meter_refresh_preset=="fast" else ""}}>Fast — 0.20 seconds</option><option value='mid' {{"selected" if cfg.audio_meter_refresh_preset=="mid" else ""}}>Mid — 0.50 seconds</option><option value='slow' {{"selected" if cfg.audio_meter_refresh_preset=="slow" else ""}}>Slow — 1.00 second</option><option value='custom' {{"selected" if cfg.audio_meter_refresh_preset=="custom" else ""}}>Custom</option></select><label>Custom refresh time (0.00–10.00 seconds)</label><input id='audioRefreshCustom' name='audio_refresh_custom' type='number' min='0' max='10' step='0.01' value='{{"%.2f"|format(cfg.audio_meter_refresh_seconds)}}'><label class='check'><input type='checkbox' name='audio_meter_smoothing' {{"checked" if cfg.audio_meter_smoothing else ""}}> Smooth the bar between received readings</label><div class='settings-note small'>0.00 uses the fastest safe browser/worker update, currently about 20 checks per second. Smoothing removes the hard jumps between readings.</div></div>
...
<div class='card'><h3>Live preview quality</h3><label>Quality preset</label><select id='previewQuality' name='preview_quality' onchange='qualityChanged()'><option value='low' {{"selected" if cfg.preview_quality=="low" else ""}}>Low — 320 px, smallest network use</option><option value='balanced' {{"selected" if cfg.preview_quality=="balanced" else ""}}>Balanced — 640 px</option><option value='high' {{"selected" if cfg.preview_quality=="high" else ""}}>High — 960 px</option><option value='very_high' {{"selected" if cfg.preview_quality=="very_high" else ""}}>Very high — 1280 px</option><option value='custom' {{"selected" if cfg.preview_quality=="custom" else ""}}>Custom</option></select><label>Custom preview width (160–1920 pixels)</label><input id='previewWidth' name='preview_width' type='number' min='160' max='1920' step='2' value='{{cfg.preview_width}}'><label>Custom JPEG quality (1 best, 31 smallest)</label><input id='previewJpegQuality' name='preview_jpeg_quality' type='number' min='1' max='31' value='{{cfg.preview_jpeg_quality}}'><div class='settings-note small'>Quality changes apply when a Preview or recording is next started. Existing recordings are not changed.</div></div>
</div>
...
const qualityValues={low:[320,12],balanced:[640,7],high:[960,4],very_high:[1280,2]};
...
def save_settings_page():
    ...
    if quality in PREVIEW_QUALITY_PRESETS:
        ...
        try:
            preview_width = max(
                160,
                min(1920, int(request.form.get("preview_width", 640))),
            )
        except (TypeError, ValueError):
            preview_width = 640
        try:
    ...
    cfg.update({
        ...
        "audio_meter_smoothing": bool(
            request.form.get("audio_meter_smoothing")
        ),
    ...
    flash(
        "Live settings saved. Browser timing applies on the next page load. "
        "Restart a running Preview to apply a new preview quality.",
        "good",
    )
...
def worker_heartbeat():
    ...
    return jsonify({"ok": True, "jobs": assigned, "runtime_settings": runtime_live_settings()})
...
if __name__ == "__main__":
    ...
    print("VIC dashboard v0.6.3 EXPERIMENTAL SOURCE FORM FIX")

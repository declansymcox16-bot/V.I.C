...
app = Flask(__name__)
app.secret_key = "vic-v0.3.8-local-dashboard"
...
<header><h1>VIC — Video Ingest Cluster v0.3.8</h1><nav><a href='/'>Dashboard</a><a href='/add'>+ Add source</a><a href='/mass-capture'>Capture Everything</a><a href='/workers'>Workers</a><a href='/live'>Live</a><a href='/live/all'>Live All</a><a href='/recordings'>Recordings</a><a href='/jobs'>Jobs</a><a href='/help'>Help</a></nav></header><main>
...
function websiteModeChanged(){const mode=document.getElementById('websiteMode');if(!mode)return;const single=document.getElementById('singleLiveOptions');const playlist=document.getElementById('playlistOptions');const upcoming=document.getElementById('upcomingOptions');if(single)single.style.display=mode.value==='single'?'block':'none';if(playlist)playlist.style.display=mode.value==='playlist'?'block':'none';if(upcoming)upcoming.style.display=mode.value==='upcoming'?'block':'none';}
document.addEventListener('DOMContentLoaded',websiteModeChanged);
...
def save_source(source_type: str):
    ...
    try:
        if source_type == "media_file":
            ...
            website_mode = form.get("website_mode", "single").strip()
            if website_mode not in {"single", "playlist", "upcoming"}:
                website_mode = "single"
            wait_min = max(5, min(3600, int(form.get("wait_min", 30))))
            wait_max = max(
                wait_min,
                min(3600, int(form.get("wait_max", 60))),
            )
            options = {
                "url": form.get("url", "").strip(),
            ...
            summary = f"{mode_label}: {options['url']}"
...
def recordings_page():
    worker_items = workers()
    body = """<div class='card'><div class='live-status-row'><div><h2 style='margin:0'>Recordings</h2><p class='muted'>These are real media files stored on worker PCs. Red file-delete controls permanently remove files and cannot be undone.</p></div>

...
def start_website_job(
    ...
    website_mode = str(options.get("website_mode", "single"))
    ...
    if job.get("mode") == "test":
        command += ["--simulate", "--no-wait-for-video"]
    ...
    if website_mode == "upcoming":
        wait_min = max(5, int(options.get("wait_min", 30)))
        wait_max = max(wait_min, int(options.get("wait_max", 60)))
        command += [
            "--wait-for-video",
            f"{wait_min}-{wait_max}",
            "--retries",
        ...
        if options.get("upcoming_live_from_start", True):
            command.append("--live-from-start")
    elif options.get("live_from_start"):
        command.append("--live-from-start")

    command += ["--", url]
    launch_process(cfg, job, command, str(folder))
...
def start_job(cfg: dict[str, Any], ffmpeg: str | None, job: dict[str, Any]) -> None:
    ...
    if job.get("mode") == "open_recordings":
        ...
        return
    if job.get("mode") == "delete_recording":

...
app = Flask(__name__)
app.secret_key = "vic-v0.6.7 EXPERIMENTAL LIVE ALL REFLOW-local-dashboard"

DASHBOARD_VERSION = "0.6.7"
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.6.7 EXPERIMENTAL LIVE ALL REFLOW folder."
        )
    return True, ""
...
#vicZoomRoot{zoom:1;width:100%;min-width:0}
...
body.vic-live-all-zoomed-out{overflow-x:hidden}
...
(function(){
  ...
  function applyZoom(value,shouldSave){
    ...
    if(root){
      ...
      // CSS zoom normally makes the same three-column layout smaller.
      // Giving Live All the inverse pre-zoom width makes the final visual
      // width fill the browser and allows CSS Grid to create more columns.
...
<div id='vicZoomRoot' class='{root_class}'><header><h1>VIC — Video Ingest Cluster v0.6.7 EXPERIMENTAL LIVE ALL REFLOW</h1><nav><a href='/'>Dashboard</a><a href='/sources/manage'>Source Library</a><a href='/add'>+ Add source</a><a href='/mass-capture'>Capture Everything</a><a href='/workers'>Workers</a><a href='/live'>Live</a><a href='/live/all'>Live All</a><a href='/health'>Health</a><a href='/recordings'>Recordings</a><a href='/storage'>Storage</a><a href='/settings'>Settings</a><a href='/tools'>Portable Tools</a><a href='/jobs'>Jobs</a><a href='/help'>Help</a></nav></header><main>
{{% with messages=get_flashed_messages(with_categories=true) %}}{{% for category,message in messages %}}<div class='flash {{{{category}}}}'>{{{{message}}}}</div>{{% endfor %}}{{% endwith %}}{body}</main></div>{GLOBAL_DISPLAY_CONTROLS}{script}{GLOBAL_DISPLAY_SCRIPT}</body></html>"""
...
def import_source_file():
    ...
    message = (
        ...
        "They have new IDs and are disabled. Edit worker/device choices, "
...
liveAllLoop();
    ...
    return render_template_string(page("Live All", body, script), jobs=data, job_ids=[item["id"] for item in data], stamp=time.time(), preview_types=PREVIEW_SOURCE_TYPES, audio_refresh_seconds=live_cfg["audio_meter_refresh_seconds"], preview_refresh_seconds=live_cfg["preview_refresh_seconds"], audio_smoothing=live_cfg["audio_meter_smoothing"], audio_motion=live_cfg["audio_meter_motion"])
...
if __name__ == "__main__":
    ...
    print("VIC dashboard v0.6.7 EXPERIMENTAL LIVE ALL REFLOW")

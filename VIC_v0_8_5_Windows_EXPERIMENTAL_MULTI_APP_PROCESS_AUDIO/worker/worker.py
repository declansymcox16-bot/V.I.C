...
def application_audio_capability() -> tuple[bool, str]:
    if os.name != "nt":
        return False, "Application/process audio is available only on Windows workers."
    build = windows_build_number()
    if build < 20348:
        return False, (
            f"Windows build {build or 'unknown'} is too old; build 20348 or later is required."
        )
    if not APPLICATION_AUDIO_HELPER.is_file():
        return False, f"Missing helper: {APPLICATION_AUDIO_HELPER}"
    return True, (
        f"Ready — Windows build {build}; captures only the selected process tree."
    )
...
def detect_devices(ffmpeg: str | None) -> dict[str, Any]:
    result: dict[str, Any] = {
        ...
        "speaker_error": "",
        "application_audio_supported": application_audio_capability()[0],
        "application_audio_status": application_audio_capability()[1],
    }
    if ffmpeg:
        try:
    ...
    if not audio_ready.wait(12.0):
        ...
        update_job(
            ...
            message=(
                ...
                + (audio_error[-1] if audio_error else "activation timed out")
            ),
        )
        return

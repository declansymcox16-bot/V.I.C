...
app = Flask(__name__)
app.secret_key = "vic-v0.8.5 EXPERIMENTAL MULTI APP PROCESS AUDIO-local-dashboard"

DASHBOARD_VERSION = "0.8.5"
...
<p class='small muted'>This does not use speaker/headphone loopback and does not record unrelated applications. It uses Windows process-loopback audio. Chrome windows/tabs can share one Chrome process tree, so another sounding Chrome tab may be included with a selected Chrome window.</p>
...
<p class='small muted'>Choose the window from the loaded list to bind its process ID. This uses Windows application/process audio, not speaker loopback. Chrome tabs/windows may share one Chrome process tree.</p>
...
def save_source(source_type: str):
    ...
    if source_type == "multi_window":
        ...
        audio_note = (
            " Each source is set to process-only application audio."
...
def build_worker_copy_zip() -> Path:
    stamp = time.strftime("%Y-%m-%d_%H-%M-%S")
    target = EXPORTS_DIR / f"VIC_Worker_Copy_v0.8.5_{stamp}.zip"

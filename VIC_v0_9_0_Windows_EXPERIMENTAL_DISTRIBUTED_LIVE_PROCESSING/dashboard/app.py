...
app = Flask(__name__)
app.secret_key = "vic-v0.9.0-distributed-live-processing-local-dashboard"
...
def source_with_defaults(item: dict[str, Any]) -> dict[str, Any]:
    ...
    result.setdefault("processing_worker_id", "same")
    result.setdefault("relay_quality", "balanced")
...
DISTRIBUTED_RELAY_SOURCE_TYPES = {
    "screen",
    "camera",
    "audio_device",
    "speaker_output",
...
RELAY_PORT_END = 39199
RELAY_QUALITY_LABELS = {
    "low_bandwidth": "Low bandwidth — about 4 Mbps per video source",
    "balanced": "Balanced — about 8 Mbps per video source",
    "high_quality": "High quality — about 15 Mbps per video source",
    "very_high": "Very high / 4K — about 25 Mbps per video source",
}


def visible_source_jobs() -> list[dict[str, Any]]:
    return [item for item in jobs() if not item.get("relay_child")]


def active_processing_count(worker_id: str) -> int:
    return sum(
        1
        for item in jobs()
        if job_is_active(item)
        and not item.get("relay_child")
        and str(item.get("processing_worker_id", item.get("worker_id", "")))
        == str(worker_id)
    )


def processing_worker_score(worker: dict[str, Any]) -> tuple[Any, ...]:
    slots = max(1, int(worker.get("processing_slots", 4) or 4))
    active = active_processing_count(str(worker.get("id", "")))
    has_gpu = bool(worker.get("gpu_devices"))
    return (
        active / slots,
        float(worker.get("cpu", 100) or 100),
        float(worker.get("memory", 100) or 100),
        not has_gpu,
        -float(worker.get("disk_free_gb", 0) or 0),
    )


def choose_processing_worker(
    requested: str,
    capture_worker: dict[str, Any],
) -> tuple[dict[str, Any] | None, str]:
    value = str(requested or "same").strip()
    if value in {"", "same"}:
    ...
    if value != "auto":
        selected = next(
            (item for item in online if str(item.get("id", "")) == value),
            None,
        )
        if not selected:
            return None, "The selected processing worker is offline."
        if (
            str(selected.get("id", ""))
            != str(capture_worker.get("id", ""))
            and not bool(selected.get("allow_remote_processing", True))
        ):
            return None, "That worker is not accepting live processing from other PCs."
        slots = max(1, int(selected.get("processing_slots", 4) or 4))
        if active_processing_count(str(selected.get("id", ""))) >= slots:
            return None, (
                f'{selected.get("display_name", "Worker")} has reached its '
                f'configured processing limit of {slots} live source(s).'
            )
        return selected, ""
    ...
    candidates = [
        ...
        for item in online
        if (
            str(item.get("id", "")) == str(capture_worker.get("id", ""))
            or bool(item.get("allow_remote_processing", True))
        )
        and active_processing_count(str(item.get("id", "")))
        < max(1, int(item.get("processing_slots", 4) or 4))
    ]
    if not candidates:
        return None, "No online worker currently has a free live-processing slot."
    return sorted(candidates, key=processing_worker_score)[0], ""


def allocate_relay_port(processing_worker_id: str) -> int | None:
    used = {
        int(item.get("relay_port", 0) or 0)
        for item in jobs()
        if job_is_active(item)
        and str(item.get("processing_worker_id", ""))
        == str(processing_worker_id)
    }
...
def worker_relay_host(worker: dict[str, Any]) -> str:
    configured = str(worker.get("relay_advertise_host", "")).strip()
    if configured:
        return configured
    address = str(worker.get("lan_ip", "")).strip()
    if address.startswith("::ffff:"):
        address = address[7:]
    if address and address not in {"127.0.0.1", "::1"}:
        return address
    return str(worker.get("host", "")).strip()
...
function applyExistingSource(){{
  ...
  const processing=document.getElementById('processingWorkerSelect');
  ensureSelectValue(
    processing,
    existingSource.processing_worker_id||'same',
    (existingSource.processing_worker_id||'same')+' — currently assigned'
  );
  setNamedField('relay_quality',existingSource.relay_quality||'balanced');
    ...
    if source_type == "multi_window":
        ...
        for index, selected in enumerate(selected_windows, start=1):
            ...
            data.append({
                ...
                "worker_id": worker_id,
                "processing_worker_id": request.form.get("processing_worker_id", "same").strip() or "same",
                "relay_quality": request.form.get("relay_quality", "balanced").strip() or "balanced",
...
def edit_source_form(source_id: str):
    ...
    worker_items = workers()
    body = """<div class='card'><h2>Edit {{source_item.name}}</h2><p class='muted'><strong>Type:</strong> {{type_name}}. The source type stays fixed; its name, worker and settings can be changed.</p>{% if active %}<p class='bad'><strong>This source is active.</strong> Stop it before saving changes.</p>{% endif %}<form method='post' action='/sources/{{source_item.id}}/edit'><label>Source name</label><input name='name' required placeholder='Friendly source name'><label>Capture worker <span class='help-tip' data-tip='The PC that physically owns or can see this source.'>?</span></label><select id='workerSelect' name='worker_id' onchange='workerChanged()'><option value='auto'>Automatic capture worker</option>{% for worker in workers %}<option value='{{worker.id}}'>{{worker.display_name}} — {{worker.host}} — {{"ONLINE" if worker.online else "OFFLINE"}}</option>{% endfor %}</select><label>Live processing worker</label><select id='processingWorkerSelect' name='processing_worker_id'><option value='same'>Same as capture worker</option><option value='auto'>Auto choose best available worker</option>{% for worker in workers %}<option value='{{worker.id}}'>{{worker.display_name}} — {{worker.processing_slots or 4}} slots — {{"ONLINE" if worker.online else "OFFLINE"}}</option>{% endfor %}</select><label>Capture-to-processing relay quality</label><select name='relay_quality'><option value='low_bandwidth'>Low bandwidth — about 4 Mbps</option><option value='balanced'>Balanced — about 8 Mbps</option><option value='high_quality'>High quality — about 15 Mbps</option><option value='very_high'>Very high / 4K — about 25 Mbps</option></select><p class='small muted'>Choose Same as capture to keep the original one-PC path. A different processing worker receives a live TCP relay and performs final FFmpeg work and file writing.</p><fieldset style='margin:14px 0'><legend>Source management</legend><label class='check'><input type='checkbox' name='enabled' {% if not source_item or source_item.enabled %}checked{% endif %}> Enabled — allow Test, Preview and Start</label><label class='check'><input type='checkbox' name='auto_reconnect' {% if source_item and source_item.auto_reconnect %}checked{% endif %}> Automatically reconnect after an unexpected source failure</label><label class='check'><input type='checkbox' name='retry_start_failures' {% if source_item and source_item.retry_start_failures %}checked{% endif %}> Automatically retry when a manual Start fails before recording begins</label><label>Retry / reconnect delay (seconds)</label><input type='number' name='reconnect_delay' min='2' max='300' value='{{source_item.reconnect_delay if source_item else 5}}'><label>Arm Auto Start availability check interval (seconds)</label><input type='number' name='auto_start_poll_seconds' min='2' max='300' value='{{source_item.auto_start_poll_seconds if source_item else 10}}'><p class='small muted'>For RTSP, direct network streams and single/upcoming website streams, Arm Auto Start checks availability without creating an empty recording. It starts recording only after real media is detected.</p><label>Recording storage</label><select name='storage_id'><option value=''>Worker default recording folder</option>{% for storage in storage_locations %}<option value='{{storage.id}}' {{"selected" if source_item and source_item.storage_id==storage.id else ""}}>{{storage.name}} — {{storage.path}} — {{storage.worker_name}}</option>{% endfor %}</select><p class='small muted'>A saved storage location also chooses the worker that owns that path. Manage locations in Storage.</p><label>After a successful recording finishes</label><select name='after_recording'><option value='keep' {{"selected" if not source_item or source_item.after_recording=="keep" else ""}}>Keep on recording worker</option><option value='copy_main' {{"selected" if source_item and source_item.after_recording=="copy_main" else ""}}>Copy recording to Main PC</option><option value='move_main' {{"selected" if source_item and source_item.after_recording=="move_main" else ""}}>Move recording to Main PC after verification</option></select><label>Notes</label><textarea name='notes' rows='3' placeholder='Device instructions, cable, audio delay, or anything useful'>{{source_item.notes if source_item else ""}}</textarea></fieldset>{{fields|safe}}<div class='inline'><button {{"disabled" if active else ""}}>Save changes</button><a class='btn' href='/'>Cancel</a></div></form></div>"""
    return render_template_string(
        page(
            "Edit source",
            body,
            source_form_script(
                {
                    ...
                    "processing_worker_id": str(source.get("processing_worker_id", "same")),
                    "relay_quality": str(source.get("relay_quality", "balanced")),
                    "options": (
                        source.get("options", {})
...
def release_relay_capture_jobs(data: list[dict[str, Any]]) -> bool:
    changed = False
    by_id = {str(item.get("id", "")): item for item in data}
    for item in data:
        if not item.get("relay_child"):
            continue
        parent = by_id.get(str(item.get("parent_job_id", "")))
        if not parent:
            if job_is_active(item):
                mark_job_stopping(item, "Processing job disappeared")
                changed = True
            continue
        if parent.get("desired_state") == "stopped" or not job_is_active(parent):
            if job_is_active(item):
                mark_job_stopping(item, "Processing job stopped")
                changed = True
            continue
        if (
            item.get("state") == "waiting_relay"
            and parent.get("state") in {"relay_listening", "running"}
        ):
            item["state"] = "pending"
            item["message"] = (
                f'Connecting lightweight capture relay to '
                f'{parent.get("processing_worker_name", "processing worker")}'
            )
            item["updated_ts"] = time.time()
            changed = True
    return changed
...
def create_job(
    ...
    *,
    ...
    capture_worker = choose_worker(str(source.get("worker_id", "auto")))
    if not capture_worker:
        return False, "No suitable online capture worker is available."

    storage = storage_by_id(str(source.get("storage_id", "")))
    requested_processing = str(source.get("processing_worker_id", "same") or "same")
    if storage:
        if not storage.get("enabled", True):
            return False, "The selected storage location is disabled."
        processing_worker = worker_by_id(str(storage.get("worker_id", "")))
        if not processing_worker or not processing_worker.get("online"):
        ...
        processing_worker, processing_error = choose_processing_worker(
            requested_processing,
            capture_worker,
        )
        if not processing_worker:
            return False, processing_error

    if str(source.get("type", "")) == "folder_watch":
        processing_worker = capture_worker
    if not processing_worker:
        return False, "No suitable processing worker is available."

    current = active_job_for_source(str(source["id"]))
    if current and not allow_active:
        return False, f'An active job already exists: {current.get("state", "unknown")}'

    capture_id = str(capture_worker.get("id", ""))
    processing_id = str(processing_worker.get("id", ""))
    capture_name = str(capture_worker.get("display_name", capture_worker.get("name", "Capture worker")))
    processing_name = str(processing_worker.get("display_name", processing_worker.get("name", "Processing worker")))
    source["capture_worker_id"] = capture_id
    source["processing_worker_id_resolved"] = processing_id

    distributed = bool(
        capture_id != processing_id
        and str(source.get("type", "")) in DISTRIBUTED_RELAY_SOURCE_TYPES
    )
    # Website URLs are network-accessible. The chosen processing worker runs
    # yt-dlp directly; there is no reason to decode and relay the webpage.
    direct_on_processing = bool(
        capture_id != processing_id
        and str(source.get("type", "")) == "website"
    )

    now = time.time()
    parent_id = uuid.uuid4().hex
    parent_state = initial_state or ("waiting" if wait_for_job_id else "pending")
    parent_message = initial_message or (
        "Waiting for previous job to stop before moving live processing"
        if wait_for_job_id
        else "Waiting for processing worker"
    )
    parent = {
        "id": parent_id,
        "source_id": source["id"],
        "source_name": source["name"],
        "worker_id": processing_id,
        "worker_name": processing_name,
        "capture_worker_id": capture_id,
        "capture_worker_name": capture_name,
        "processing_worker_id": processing_id,
        "processing_worker_name": processing_name,
        "distributed_processing": distributed,
        "direct_on_processing": direct_on_processing,
        "relay_role": "processing" if distributed else "",
        "source": source,
        "mode": mode,
    ...
    data = jobs()
    if distributed:
        port = allocate_relay_port(processing_id)
        if port is None:
            return False, (
                f'{processing_name} has no free VIC relay port in the '
                f'{RELAY_PORT_START}-{RELAY_PORT_END} range.'
            )
        relay_host = worker_relay_host(processing_worker)
        if not relay_host:
            return False, (
                f'VIC could not determine a LAN address for {processing_name}. '
                "Restart that worker or set Relay advertised address in Worker Setup."
            )
        parent.update({
            "relay_port": port,
        ...
        child = {
            "id": uuid.uuid4().hex,
            "source_id": source["id"],
            "source_name": source["name"] + " — lightweight capture relay",
            "worker_id": capture_id,
            ...
            "desired_state": "running",
            "state": "waiting_relay",
            "message": f"Waiting for {processing_name} to open relay port {port}",
            "output": f"Live TCP relay to {relay_host}:{port}",
            "created_ts": now + 0.001,
            "updated_ts": now,
        }
        data.extend([parent, child])
    else:
        data.append(parent)
    ...
    if distributed:
        return True, (
            f'{mode.title()} queued: capture on {capture_name} → '
            f'live processing and recording on {processing_name}.'
        )
    if direct_on_processing:
        return True, (
            f'{mode.title()} website job sent directly to processing worker '
            f'{processing_name}.'
        )
    return True, f'{mode.title()} job sent to {processing_name}.'
...
@app.post("/sources/<source_id>/move-processing")
def move_source_processing(source_id: str):
    source = source_by_id(source_id)
    if not source:
        flash("That source no longer exists.", "bad")
        return action_redirect("/live/all")

    requested = str(request.form.get("processing_worker_id", "same") or "same").strip()
    if requested not in {"same", "auto"} and not worker_by_id(requested):
        flash("That processing worker is no longer registered.", "bad")
        return action_redirect("/live/all")
    ...
    for item in data_sources:
        if str(item.get("id", "")) == source_id:
            item["processing_worker_id"] = requested
            item["modified"] = time.strftime("%Y-%m-%d %H:%M:%S")
            updated_source = source_with_defaults(item)
            break
    if updated_source is None:
        flash("Source not found.", "bad")
        return action_redirect("/live/all")
    save_json(SOURCES_FILE, data_sources)

    current = active_job_for_source(source_id)
    if not current:
        flash(
            "Processing-worker choice saved. It will be used the next time this source starts.",
            "good",
        )
        return action_redirect("/live/all")
    ...
    for item in data_jobs:
        if item.get("source_id") == source_id and job_is_active(item):
            mark_job_stopping(item, "Moving live processing to another worker")
    save_json(JOBS_FILE, data_jobs)

    original_mode = str(current.get("mode", "record"))
    if original_mode not in {"record", "preview", "test"}:
        original_mode = "record"
    ok, message = create_job(
        updated_source,
        original_mode,
        allow_active=True,
        ...
        initial_message="Waiting for the old live-processing path to close safely",
    )
    flash(
        ("Processing move queued. " + message) if ok else ("Could not queue the new processing worker: " + message),
        "good" if ok else "bad",
    )
    return action_redirect("/live/all")
...
<div class='health-grid'>{% for job in jobs %}<div class='health-card' id='health-card-{{job.id}}'><div class='live-status-row'><div><h3 style='margin:0'>{{job.source_name}}</h3><span class='small muted'>{% if job.distributed_processing %}Capture: {{job.capture_worker_name}} → Processing: {{job.processing_worker_name}}{% else %}{{job.worker_name}}{% endif %}</span></div><span id='health-mode-{{job.id}}' class='state-pill'>{{job.mode|upper}} · {{job.state|upper}}</span></div><div class='health-metrics'>
...
def build_worker_copy_zip() -> Path:
    stamp = time.strftime("%Y-%m-%d_%H-%M-%S")
    target = EXPORTS_DIR / f"VIC_Worker_Copy_v0.9.0_{stamp}.zip"
        ...
        result.append(
            {
                ...
                "capture_worker_name": item.get("capture_worker_name", item.get("worker_name", "")),
                "processing_worker_name": item.get("processing_worker_name", item.get("worker_name", "")),
                "distributed_processing": bool(item.get("distributed_processing")),
                "state": item.get("state", "unknown"),
                "mode": item.get("mode", ""),
                "message": item.get("message", ""),
...
def worker_heartbeat():
    ...
    entry = {
        ...
        "lan_ip": str(payload.get("relay_advertise_host", "")).strip() or str(request.remote_addr or previous.get("lan_ip", "")),
        "relay_advertise_host": payload.get("relay_advertise_host", previous.get("relay_advertise_host", "")),
        "processing_slots": max(1, min(64, int(payload.get("processing_slots", previous.get("processing_slots", 4)) or 4))),
        "allow_remote_processing": bool(payload.get("allow_remote_processing", previous.get("allow_remote_processing", True))),
    ...
    if release_relay_capture_jobs(current_jobs):
        current_jobs_changed = True
...
def job_update():
    ...
    for item in data:
        if item.get("id") == job_id:
            ...
            should_retry_start = bool(
                not item.get("relay_child")
                and incoming_state == "failed"
                and item.get("mode") == "record"
    ...
    # Synchronise the hidden lightweight capture job with its visible
    # processing job. The parent remains the only job shown in Live views.
    if matched_job and matched_job.get("relay_child"):
        parent_id = str(matched_job.get("parent_job_id", ""))
        parent = next(
            (item for item in data if str(item.get("id", "")) == parent_id),
            None,
        )
        child_state = str(matched_job.get("state", ""))
        if parent and child_state == "running":
            parent["state"] = "running"
            parent["ever_running"] = True
            parent["message"] = (
                f'Live relay connected: {parent.get("capture_worker_name", "Capture")} '
                f'→ {parent.get("processing_worker_name", "Processing")}'
            )
            parent["updated_ts"] = time.time()
        elif parent and child_state in {"failed", "finished", "stopped"} and parent.get("desired_state") == "running":
            parent["desired_state"] = "stopped"
            parent["state"] = "stopping"
            parent["message"] = (
                "The capture relay ended unexpectedly. Stopping the processing "
                "listener safely. " + str(matched_job.get("message", ""))
            )
            parent["updated_ts"] = time.time()

    if matched_job and not matched_job.get("relay_child") and matched_job.get("distributed_processing"):
        parent_state = str(matched_job.get("state", ""))
        if parent_state in {"failed", "finished", "stopped", "stopping"} or matched_job.get("desired_state") == "stopped":
            for child in data:
                if str(child.get("parent_job_id", "")) != str(matched_job.get("id", "")):
                    continue
                if job_is_active(child):
                    mark_job_stopping(child, "Processing worker stopped")

...
app = Flask(__name__)
app.secret_key = "vic-v0.4.5-local-dashboard"

DASHBOARD_VERSION = "0.4.5"
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.4.5 folder."
        )
    return True, ""
...
<header><h1>VIC — Video Ingest Cluster v0.4.5</h1><nav><a href='/'>Dashboard</a><a href='/add'>+ Add source</a><a href='/mass-capture'>Capture Everything</a><a href='/workers'>Workers</a><a href='/live'>Live</a><a href='/live/all'>Live All</a><a href='/health'>Health</a><a href='/recordings'>Recordings</a><a href='/jobs'>Jobs</a><a href='/help'>Help</a></nav></header><main>
...
def preview_all_sources():
    configured = [item for item in sources() if source_supports_preview(item)]
    if not configured:
...
def retry_transfer(transfer_id: str):
    ...
    if not source_ready or not target_ready:
        flash(
            source_reason if not source_ready else target_reason,
            "bad",
        )
        return redirect("/recordings")
    ...
    if not inventory:
        flash(
            "The source file is not currently reported by that worker. "
            "Wait for its recording list to refresh.",
            "bad",
        )
        return redirect("/recordings")
    ...
    update_transfer(
        ...
        state="queued_upload",
        message=(
            "Retry queued. Waiting for the v0.4.5 source worker "
    ...
    flash(
        "Transfer retry queued. The source worker should change it to "
        "WORKER QUEUE within a few seconds.",
        "good",
    )
    return redirect("/recordings")
...
<div><h3 style='margin:0'>{{worker.display_name}}</h3><p class='muted'><code>{{worker.recordings_root or "Recording folder not reported"}}</code> · {{"ONLINE" if worker.online else "OFFLINE"}} · VIC {{worker.worker_version or "older/unknown"}} · {{worker.recordings|length}} file(s)</p></div>
<div class='inline'>

...
ENCODER_PROBE_CACHE: dict[tuple[str, str], bool] = {}
...
def encoder_is_available(ffmpeg: str, encoder: str) -> bool:
    key = (ffmpeg, encoder)
    if key in ENCODER_PROBE_CACHE:
        return ENCODER_PROBE_CACHE[key]
    try:
        probe = subprocess.run(
            [
                ffmpeg, "-hide_banner", "-loglevel", "error",
                "-f", "lavfi", "-i", "color=c=black:s=128x72:r=30",
        ...
        available = False
    ENCODER_PROBE_CACHE[key] = available
    return available
...
def select_video_encoder(
    ...
    if preference != "cpu":
        for name in order:
            ...
            if encoder_is_available(ffmpeg, encoder):
                return args + ["-pix_fmt", "yuv420p"], label
    return ["-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-pix_fmt", "yuv420p"], "CPU x264"

...
app = Flask(__name__)
app.secret_key = "vic-v0.4.9-local-dashboard"

DASHBOARD_VERSION = "0.4.9"
...
def transfer_worker_ready(
    ...
    if version_tuple(current) < MIN_TRANSFER_WORKER_VERSION:
        ...
        return False, (
            ...
            "START_WORKER.bat from the v0.4.9 folder."
        )
    return True, ""
...
def retry_transfer(transfer_id: str):
    ...
    update_transfer(
        ...
        state="queued_upload",
        message=(
            "Retry queued. Waiting for the v0.4.8 source worker "
...
def transfer_progress(transfer_id: str):
    ...
    if "bytes_done" in fields:
        ...
        if current_bytes >= previous_bytes and elapsed >= 0.15:
            ...
            smoothed = instant if previous_rate <= 0 else previous_rate * 0.65 + instant * 0.35
...
def transfer_status_api():
    ...
    return jsonify({"transfers": result, "server_ts": time.time()})

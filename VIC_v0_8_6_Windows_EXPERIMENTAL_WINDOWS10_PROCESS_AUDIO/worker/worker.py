...
def finalise_application_audio_capture(
    ...
    if local.get("application_audio_finalised"):
        return (
            str(local.get("final_output", local.get("output", ""))),
    ...
    if not ffmpeg or not video.is_file() or not audio.is_file():
        ...
        message = (
            "Application audio was kept as a companion WAV because the final merge inputs were unavailable."
        )
    ...
    offset = float(local.get("application_audio_offset_seconds", 0.0) or 0.0)
    command = [
        ffmpeg,
        "-hide_banner",
        "-loglevel",
    ...
    try:
        ...
        if completed.returncode != 0 or not temporary.is_file() or temporary.stat().st_size <= 0:
            raise RuntimeError(
                (completed.stderr or "FFmpeg did not create the merged recording").strip()
            )
        os.replace(temporary, final)
        video.unlink(missing_ok=True)
        audio.unlink(missing_ok=True)
        local["output"] = str(final)
        local["output_paths"] = [str(final)]
        message = "Application process audio was merged into the final MKV."
        local["application_audio_finalise_message"] = message
        ...
        output = f"{video} | {audio}"
        message = (
            "Could not merge application audio automatically, so VIC kept the original video and WAV safely: "
            + str(exc)
        )
        local["application_audio_finalise_message"] = message
        return output, message
        ...
        try:
            ...
            update_job(
                ...
                message=(
                    ...
                    if preview_mode
                    else f"Recording window with process-only application audio from {process_name or 'PID ' + str(target_pid)}"
                    if mode == "record"

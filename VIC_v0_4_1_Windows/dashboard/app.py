...
app = Flask(__name__)
app.secret_key = "vic-v0.4.1-local-dashboard"
...
<header><h1>VIC — Video Ingest Cluster v0.4.1</h1><nav><a href='/'>Dashboard</a><a href='/add'>+ Add source</a><a href='/mass-capture'>Capture Everything</a><a href='/workers'>Workers</a><a href='/live'>Live</a><a href='/live/all'>Live All</a><a href='/recordings'>Recordings</a><a href='/jobs'>Jobs</a><a href='/help'>Help</a></nav></header><main>
...
def create_recording_transfer():
    source_worker_id = str(request.form.get("source_worker_id", "")).strip()
    target_worker_id = str(request.form.get("target_worker_id", "")).strip()
    source_path = str(request.form.get("path", "")).strip()
    operation = str(request.form.get("operation", "move")).strip().lower()
    ...
    inventory_item = next(
        (
            ...
            if str(item.get("path", "")) == source_path
        ),
        None,
    )
    if not inventory_item:
        flash(
            "That file is no longer in the worker's reported recording list. "
            "Wait a few seconds and refresh the Recordings page.",
            "bad",
        )
        return redirect("/recordings")
    ...
    record = {
        ...
        "state": "queued_upload",
        "progress_percent": 0,
        "message": "Waiting for the source worker to upload the file",
        "source_worker_id": source_worker_id,
        "source_worker_name": source_worker.get("display_name", source_worker.get("name", "Source worker")),
        "target_worker_id": target_worker_id,
        "target_worker_name": target_worker.get("display_name", target_worker.get("name", "Destination worker")),
        "source_path": source_path,
        "relative": str(inventory_item.get("relative", inventory_item.get("name", "recording.bin"))),
        "filename": str(inventory_item.get("name", Path(source_path).name)),
    ...
    job = create_worker_control_job(
        ...
        f'{operation.title()} recording to {record["target_worker_name"]}: {record["filename"]}',
        {
    ...
    update_transfer(transfer_id, source_job_id=job["id"])
    flash(
        f'{operation.title()} queued: {record["filename"]} → '
        f'{record["target_worker_name"]}. The original is kept until the '
        "destination verifies the complete file.",
        "good",
    )
    return redirect("/recordings")
...
def recordings_page():
    worker_items = workers()
    online_workers = [item for item in worker_items if item.get("online")]
    ...
    body = """<div class='card'><div class='live-status-row'><div><h2 style='margin:0'>Recordings and file transfers</h2><p class='muted'>Move or copy recordings between worker PCs. <strong>Move</strong> deletes the original only after the destination verifies the complete file. Transfers temporarily pass through the main Dashboard PC.</p></div>
...
{% if transfers %}<div class='card'><div class='live-status-row'><div><h3 style='margin:0'>File transfers</h3><p class='muted'>The main PC safely relays and verifies each file. A failed move leaves the original untouched.</p></div><form method='post' action='/transfers/clear-finished'><button>Clear finished transfers</button></form></div>
<table><tr><th>File</th><th>From → To</th><th>Action</th><th>Status</th><th>Progress</th><th>Destination</th></tr>{% for transfer in transfers %}<tr><td><strong>{{transfer.filename}}</strong><br><span class='small muted'>{{transfer.reported_size_mb}} MB</span></td><td>{{transfer.source_worker_name}}<br>→ {{transfer.target_worker_name}}</td><td>{{transfer.operation|upper}}</td><td class='{{"good" if transfer.state=="completed" else "bad" if transfer.state in ["failed","source_delete_failed"] else "muted"}}'><strong>{{transfer.state.replace("_"," ")|upper}}</strong><br><span class='small'>{{transfer.message}}</span></td><td><div class='meter dashboard-meter'><span style='width:{{transfer.progress_percent or 0}}%'></span></div><span class='small'>{{transfer.progress_percent or 0}}%</span></td><td><code>{{transfer.destination_path or "—"}}</code></td></tr>{% endfor %}</table></div>{% endif %}
{% for worker in workers %}<div class='card'><div class='inline' style='justify-content:space-between'><div><h3 style='margin:0'>{{worker.display_name}}</h3><p class='muted'><code>{{worker.recordings_root or "Recording folder not reported"}}</code> · {{"ONLINE" if worker.online else "OFFLINE"}}</p></div><div class='inline'><form method='post' action='/workers/{{worker.id}}/open-recordings'><button>Open folder on worker</button></form><form method='post' action='/workers/{{worker.id}}/delete-recordings-all' onsubmit='return confirm("PERMANENTLY DELETE ALL recording files from {{worker.display_name}}? This cannot be undone.")'><button class='control-stop' {{"disabled" if not worker.online else ""}}>Delete All From This Worker</button></form></div></div>
<table><tr><th>File</th><th>Folder</th><th>Size</th><th>Modified</th><th>Transfer</th><th>Delete</th></tr>{% for item in worker.recordings or [] %}<tr class='recording-row'><td><strong>{{item.name}}</strong><br><code class='small'>{{item.path}}</code></td><td>{{item.relative}}</td><td>{{item.size_mb}} MB</td><td>{{item.modified}}</td><td>{% if worker.online and online_workers|length > 1 %}<form method='post' action='/recordings/transfer' onsubmit='return confirm("Transfer {{item.name}}? A Move deletes the original only after verification.")'><input type='hidden' name='source_worker_id' value='{{worker.id}}'><input type='hidden' name='path' value='{{item.path}}'><select name='target_worker_id' required>{% for target in online_workers %}{% if target.id != worker.id %}<option value='{{target.id}}'>{{target.display_name}}{{" — Main PC" if target.is_local_dashboard else ""}}</option>{% endif %}{% endfor %}</select><div class='inline' style='margin-top:7px'><button name='operation' value='copy' title='Keep the original and make a verified copy'>Copy to worker</button><button class='control-start' name='operation' value='move' title='Delete the original only after the destination verifies the file'>Move to worker</button></div></form>{% if main_worker and worker.id != main_worker.id and main_worker.online %}<form method='post' action='/recordings/transfer' style='margin-top:7px' onsubmit='return confirm("Move {{item.name}} to the Main PC recording folder? The original is deleted only after verification.")'><input type='hidden' name='source_worker_id' value='{{worker.id}}'><input type='hidden' name='target_worker_id' value='{{main_worker.id}}'><input type='hidden' name='path' value='{{item.path}}'><button class='control-start' name='operation' value='move'>Move to Main PC</button></form>{% endif %}{% else %}<span class='small muted'>Another online worker is required.</span>{% endif %}</td><td><form method='post' action='/workers/{{worker.id}}/delete-recording' onsubmit='return confirm("PERMANENTLY delete {{item.name}} from the worker PC?")'><input type='hidden' name='path' value='{{item.path}}'><button class='control-stop' {{"disabled" if not worker.online else ""}}>Delete file</button></form></td></tr>{% else %}<tr><td colspan='6' class='muted'>No recordings reported by this worker.</td></tr>{% endfor %}</table></div>{% else %}<div class='card muted'>No worker information is available.</div>{% endfor %}"""
    return render_template_string(
        page("Recordings", body),
        workers=worker_items,

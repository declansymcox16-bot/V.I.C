...
app = Flask(__name__)
app.secret_key = "vic-v0.3.6-local-dashboard"
...
.live-card{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:15px;min-width:0}
...
<header><h1>VIC — Video Ingest Cluster v0.3.6</h1><nav><a href='/'>Dashboard</a><a href='/add'>+ Add source</a><a href='/mass-capture'>Capture Everything</a><a href='/workers'>Workers</a><a href='/live'>Live</a><a href='/live/all'>Live All</a><a href='/recordings'>Recordings</a><a href='/jobs'>Jobs</a><a href='/help'>Help</a></nav></header><main>
...
def latest_live_jobs() -> list[dict[str, Any]]:
    """Return the newest job for every source, with active jobs first."""
...
<h2>Live source monitor</h2>
<p class='muted'>The audio meters on this page update several times per second. Open an individual source, or use Live All to see every latest source together.</p>
<table><tr><th>Source</th><th>Worker</th><th>State</th><th style='min-width:230px'>Audio</th><th>Open</th></tr>
{% for job in jobs %}<tr>
...
<td><a class='btn' target='_blank' href='/live/{{job.id}}'>Open individual live tab</a></td>
...
<div class='live-status-row'><div><h2 style='margin:0'>Live All</h2><p class='muted'>Every latest source appears together. Video previews refresh about once per second and audio meters update several times per second.</p></div><button type='button' onclick='document.documentElement.requestFullscreen?.()'>Full screen</button></div>
</div>
<div class='live-grid'>
{% for job in jobs %}
<div class='live-card' id='live-card-{{job.id}}'>
...
<a class='btn' target='_blank' href='/live/{{job.id}}'>Open individual live view</a>
</div>
